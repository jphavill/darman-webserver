...
def list_sprint_comparison(
    ...
    if not unique_person_ids:
        raise HTTPException(status_code=422, detail="at least one person_id is required")
    ...
    if len(people) != len(unique_person_ids):
        raise HTTPException(status_code=422, detail="one or more person_ids are invalid")
